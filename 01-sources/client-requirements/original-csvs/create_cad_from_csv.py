#!/usr/bin/env python3
"""
Create Client Anchor Documents (CAD) from cleaned CSV files.
...
def parse_csv_to_requirements(csv_file, doc_title, doc_id):
    ...
    with open(csv_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if not req_id or not req_id.startswith(('SW-', 'SWP-')):
                continue
...
def main():
    """Create CAD-002 from SW-cleaned.csv"""

    # Paths
    base_path = Path("/Users/andrewbailey/dev/block-sw-discovery/client-anchor-docs")
    csv_file = base_path / "original-csvs" / "SW-cleaned.csv"
    json_output = base_path / "CAD-002-sw-vendor-requirements.json"
    md_output = base_path / "CAD-002-sw-vendor-requirements.md"

    # Document metadata
    doc_id = "CAD-002"
    doc_title = "Sourcing Window (Vendor) Requirements"
    source_file = "SW-cleaned.csv (from original Excel export)"
